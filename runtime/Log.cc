                            ...
                            int pid)
{
    ...
    while (remaining > 0) {
        ...
        // std::cout << "info.dumpDirectFunction" << std::endl;
        info.dumpDirectFunction(fileHandler, info ,entry,bufferId, checkPointForDumpTxtLog,&printfBuf, printfBufSize,hostname,pid);
...
printSingleArg(FILE *outputFd,
               ...
               int precision = -1)
{
    std::cout << "printSingleArg" << std::endl;
                                        ...
                                        void (*aggregationFn)(const char*, ...))
{
    ...
    std::cout << "decompressNextLogStatement" << std::endl;
    if (outputFd) {
...
//            timeDiff = 0;
//
       fprintf(outputFd, "%4ld) ", logMsgsProcessed);
...
Log::Decoder::internalDecompressUnordered(FILE* outputFd,
                                        ...
                                        void(*aggregationFn)(const char*,...))
{
    std::cout << "internalDecompressUnordered" << std::endl;
...
{
    std::cout << "decompressTo" << std::endl;
    if (filename.empty() || !inputFd) return -1;
