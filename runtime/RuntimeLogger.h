 ...
 */
    ...
    public:
        ...
        static void setLogFile(const char *hostname, int pid);
    ...
    PRIVATE:
        ...
        void compressionThreadMain(int i,const char *hostname, int pid);

        void setLogFile_internal(const char *hostname, int pid);

        const char * getLogFile_internal() const;
        void modifyFileHandleTask(Log::Encoder &encoder);

        void waitForAIO();

        void trim(std::string& s);

        bool nano_conf(const std::string& filename, NanoConfig& config);
        /**
        ...
        // Add LogSplitter member
        std::shared_ptr<LogSplitter> logSplitter_;
