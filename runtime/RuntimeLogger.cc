...
RuntimeLogger::RuntimeLogger()
        ...
        , logSplitter_(nullptr)
{
    // std::cout << "// RuntimeLogger constructor " << std::endl;
...
void RuntimeLogger::modifyFileHandleTask(Log::Encoder &encoder)
{
    
    while (!stopFlag_.load())
    {
        fileMutex_.lock();
        FILE *filehandler = fopen(RuntimeLogger::getTxtLogFile(), "a");
        encoder.setFileHandler(filehandler);
        fileMutex_.unlock();
        // std::cout << "stopFlag_    " << stopFlag_  << std::endl;
        // 这里可能带来一些潜在的性能问题
        std::this_thread::sleep_for(std::chrono::seconds(2));
        // std::cout << "RuntimeLogger::modifyFileHandleTask" << std::endl;
    }
}
...
*/
void
RuntimeLogger::compressionThreadMain(int i, const char * hostname, int pid)
{
    ...
    std::thread modifyHandleTask; 
    if (i == 1)
    {
        modifyHandleTask = std::thread([this, &encoder]()
                                       { modifyFileHandleTask(encoder); });
    }
    ...
    {
        coreId = sched_getcpu();
        // std::cout << " compressionThreadMain()" << std::endl;
                ...
                if (peekBytes > 0) {
                    ...
                    while (remaining > 0) {
                        long bytesToEncode = std::min(
                                ...
                                remaining);
                        // std::cout << "compressionThreadMain ddd" << remaining  << std::endl;
        ...
        // Check if we need to rotate log file before writing
        if (logSplitter_) {
            if (logSplitter_->checkAndRotate(bytesToWrite)) {
                
                // Only update file descriptor if rotation occurred
                aioCb.aio_fildes = fileno(logSplitter_->getCurrentFileHandler());
                // Add write operation after rotation
                if (aio_write(&aioCb) == -1) {
                    fprintf(stderr, "Error at aio_write(): %s\n", strerror(errno));
                }
                hasOutstandingOperation = true;
            } else {
                // Normal write when no rotation needed
                if (aio_write(&aioCb) == -1) {
                    fprintf(stderr, "Error at aio_write(): %s\n", strerror(errno));
                }
                hasOutstandingOperation = true;
            ...
            // Original write logic when no splitter
            if (aio_write(&aioCb) == -1) {
                fprintf(stderr, "Error at aio_write(): %s\n", strerror(errno));
            }
            hasOutstandingOperation = true;
    ...
    // std::cout << "end      compression" <<  std::endl;
    stopFlag_.store(true);
    cycleAtThreadStart = 0;
    cyclesActive += PerfUtils::Cycles::rdtsc() - cyclesAwakeStart;
    if (i == 1)
    {
        modifyHandleTask.join();
    }
...
void RuntimeLogger::setLogFile_internal(const char *hostname, int pid)
{
    ...
    if (outputFd > 0) {
        close(outputFd);
        remove(NanoLogConfig::DEFAULT_LOG_FILE);
    }
    ...
    compressionThread = std::thread([this, i, hostname, pid]()
                                    { compressionThreadMain(i, hostname, pid); });
#endif
}
