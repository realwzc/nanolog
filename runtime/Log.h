...
class LogSplitter;  // Forward declaration
...
struct StaticLogInfo {
    ...
    typedef void (*DumpDirectFn)(FILE *, const StaticLogInfo&, Log::UncompressedEntry*,uint32_t, const Log::Checkpoint &, char **, int32_t &, const char*, int);
...
namespace Log {
    ...
    PUBLIC:
        ...
        long encodeLogMsgs(char *from, uint64_t nbytes,
                           ...
                           uint64_t *numEventsCompressedconst ,
                           const char *hostname,
                           int pid);
        ...
        FILE *getFileHandler() { return fileHandler; };
        void setFileHandler(FILE *hander) { fileHandler = hander; };
        PRIVATE : 
