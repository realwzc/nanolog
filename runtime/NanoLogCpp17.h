...
inline void printfLogHeader(FILE * fHandler, const StaticLogInfo&info, Log::UncompressedEntry*entry,uint32_t threadId, const Log::Checkpoint & checkPoint,const char *hostname, int pid)
{
    std::string timeString = getTimeString(entry->timestamp, checkPoint);
    int32_t logLevel = info.severity;
    auto filename = info.filename;
    auto lineNum = info.lineNum;
    std::string newTimeString = timeString.substr(0, 23);

    static const char* logLevelNames[] = {"(none)", "ERROR", "WARNING",
                                        "INFO", "DEBUG"};

    fprintf(fHandler, "%s CST|%s|%s|%d|%d||%s:%d ", newTimeString.c_str(), logLevelNames[logLevel], hostname,pid, threadId,filename, lineNum);
}

template<typename... Ts>
inline void __attribute__((always_inline)) printLog(FILE * fHandler, const StaticLogInfo&info, Log::UncompressedEntry*entry, uint32_t threadId,const Log::Checkpoint & checkPoint,char **printfBuf, int &printfBufSizeTotal, const char* hostname, int pid)
{
    ...
    // std::cout << "printLog" << info.formatString << std::endl;
    printfLogHeader(fHandler, info, entry,threadId, checkPoint,hostname,pid);
...
template<typename... Ts>
inline void dumpDirect(FILE * fHandler, const StaticLogInfo&info, Log::UncompressedEntry*entry, uint32_t threadId,const Log::Checkpoint & checkPoint,char **printfBuf, int &printfBufSizeTotal,const char* hostname, int pid) 
{
    // std::cout << "dumpDirect" << hostname << pid << std::endl;
    printLog<Ts...>(fHandler, info, entry, threadId, checkPoint, printfBuf, printfBufSizeTotal, hostname, pid);
}
//end of adding
